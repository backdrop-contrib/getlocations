...
<?php
  ...
  foreach (array_keys($data) AS $k) {
    ...
    $output .= '<td><a href="#" onClick="Backdrop.getlocations_marker_box.getlocations_marker_get(\'' . $data[$k]['machine_name'] . '\', \'' . $linktype . '\'); return false;" ><img src="' . base_path() .  $data[$k]['path'] . '" title="' . $data[$k]['display_name'] . '"></a></td>';
    $ct++;
    if ($ct >= $numcols) {
      $output .= '</tr>';
      $ct = 0;
